...
(cd $OPENSSL_FULL_VERSION;
 sed -i -e 's/\-mandroid//g' Configurations/10-main.conf
 ...
 OPENSSL_CONFIGURE_OPTIONS="no-shared no-pic no-krb5 no-idea no-camellia \
        ...
        no-ssltrace no-tlsext no-mdc2 no-ecdh no-engine no-unit-test \
 ...
 for ANDROID_TARGET_PLATFORM in $ARCH #arm64-v8a
 do
